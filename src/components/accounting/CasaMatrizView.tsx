...
export function CasaMatrizView() {
  ...
  const party = parties[0]; // single party (JORD AB) v1

  const { data: balance } = useQuery({
    queryKey: ["home-office-balance", party?.id, selectedEntityId],
    queryFn: async () => {
      let q = supabase
        .from("home_office_balance")
        .select("*")
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              {party.name} — Casa Matriz
            </CardTitle>
            <p className="text-sm text-muted-foreground mt-1">
              Préstamo en {party.currency} · Interés {Number(party.interest_rate_pct).toFixed(2)}% ({party.interest_basis})
            </p>
          </div>
          {canWrite && (
            <div className="flex gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setRepayOpen(true)}>Registrar repago</DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => accrueNow.mutate()} disabled={accrueNow.isPending}>
                    {accrueNow.isPending ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-1 h-4 w-4" />}
                    Devengar interés del mes
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Stat label={`Principal (${party.currency})`} value={fmtMoney(principalFc, party.currency)} />
            <Stat label="Principal (DOP histórico)" value={fmtMoney(principalDop, "DOP")} />
            <Stat label="Interés devengado (DOP)" value={fmtMoney(accruedDop, "DOP")} />
            <Stat
              label="FX no realizado"
              value={unrealizedFx === null ? "—" : fmtMoney(unrealizedFx, "DOP")}
              tone={unrealizedFx === null ? "muted" : unrealizedFx >= 0 ? "neg" : "pos"}
              hint={usdRate ? `Tasa hoy ${Number(usdRate).toFixed(4)}` : undefined}
            />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle>Aportes de Casa Matriz</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Monto FC</TableHead>
                <TableHead className="text-right">Tasa</TableHead>
                <TableHead className="text-right">DOP</TableHead>
                <TableHead className="text-right">Saldo FC</TableHead>
      ...
      {repayments.length > 0 && (
        <Card>
          <CardHeader><CardTitle>Repagos</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Tasa</TableHead>
                  <TableHead className="text-right">DOP</TableHead>
                  <TableHead className="text-right">FX realizado</TableHead>
                  <TableHead>Referencia</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      <Card>
        <CardHeader><CardTitle>Devengos de interés</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Tasa %</TableHead>
                <TableHead className="text-right">Interés FC</TableHead>
                <TableHead className="text-right">Interés DOP</TableHead>
            ...
            <TableBody>
              {accruals.length === 0 && (
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-6">Sin devengos</TableCell></TableRow>
              )}
              {accruals.map((a) => (
                <TableRow key={a.id}>
