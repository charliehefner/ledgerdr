...
export function EditInternalTransferDialog({ transaction, bankAccounts, open, onOpenChange, onSaved }: Props) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (transaction && open) {
      ...
      setToAccount(transaction.destination_acct_code || "");
  ...
  const toAcct = bankAccounts.find((a) => a.id === toAccount);
  ...
  const isValid = () => {
    if (!date || !fromAccount || !toAccount || !amount) return false;
  ...
  const handleSave = async () => {
    if (!transaction || !isValid()) {
      if (fromAccount === toAccount) toast.error("Origen y destino no pueden ser iguales");
      else if (isCrossCurrency && !destAmount) toast.error("Ingrese monto destino para transferencia multi-moneda");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Editar Transferencia Interna</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Select value={fromAccount || undefined} onValueChange={setFromAccount}>
                ...
                <SelectContent className="bg-popover max-h-[300px]">{renderAccountOptions(false)}</SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select value={toAccount || undefined} onValueChange={setToAccount}>
                <SelectTrigger><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                <SelectContent className="bg-popover max-h-[300px]">{renderAccountOptions(true)}</SelectContent>
              </Select>
              {toAcct && <p className="text-xs text-muted-foreground">Moneda: {toCur}</p>}
            </div>
          </div>
