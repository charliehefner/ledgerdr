...
export function IR17ReportView() {
  ...
  // Section II: Employee benefits (retribuciones complementarias)
    ...
    const rows = employees
      .map((emp) => {
        ...
        return {
          ...
          name: emp.name,
          cedula: emp.cedula,
          benefitTypes: empBenefits.map((b) => b.benefit_type).join(", "),
          monthlyAmount: monthlyBenefits,
          tax,
        };
      })
      .filter(Boolean) as { id: string; name: string; cedula: string; benefitTypes: string; monthlyAmount: number; tax: number }[];
  ...
  const handleExportExcel = async () => {
    ...
    // Sheet 1: ISR Retenciones a terceros
    ...
    // Sheet 2: Retribuciones complementarias
    const ws2 = wb.addWorksheet("II - Retrib. Complementarias");
    ws2.columns = [
      { header: "Cédula", key: "cedula", width: 15 },
      { header: "Nombre", key: "name", width: 30 },
      { header: "Tipo Beneficio", key: "type", width: 25 },
      { header: "Monto Mensual", key: "amount", width: 18 },
      { header: "Impuesto 27%", key: "tax", width: 15 },
    ];
    sectionII.rows.forEach((r) =>
      ws2.addRow({ cedula: r.cedula, name: r.name, type: r.benefitTypes, amount: r.monthlyAmount, tax: r.tax })
    );
    ws2.addRow({ cedula: "", name: "TOTAL", type: "", amount: 0, tax: sectionII.totalTax });

    // Sheet 3: ITBIS retenido
          ...
          <div className="rounded-md border overflow-auto">
            <Table>
              ...
              <TableBody>
                {sectionII.rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-xs">{r.benefitTypes}</TableCell>
              ...
              {sectionII.rows.length > 0 && (
                <TableFooter>
                  <TableRow>
                    <TableCell colSpan={4} className="font-semibold">Subtotal retribuciones complementarias</TableCell>
