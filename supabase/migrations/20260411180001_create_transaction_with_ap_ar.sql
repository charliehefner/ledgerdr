-- Atomic transaction + AP/AR document creation.
-- Replaces the two-step frontend pattern (createTransaction + separate insert)
-- with a single PL/pgSQL function so both rows are inside one DB transaction.
-- Business rules from apArUtils.ts are encoded here as the authoritative copy.
...
BEGIN
  ...
  IF p_cbs_code IS NOT NULL THEN
    SELECT id INTO v_cbs_id
    FROM public.cbs_codes
    WHERE code = p_cbs_code
    LIMIT 1;
  END IF;
  ...
  -- ── AP/AR creation rules (mirrors apArUtils.ts shouldCreateApAr) ─────────
  ...
  IF v_should_create_ap_ar THEN
    ...
    -- GL account code (mirrors getApArAccountCode)
    v_acct_code := CASE
    ...
    FROM public.chart_of_accounts
    WHERE account_code = v_acct_code
      AND allow_posting = true
      AND deleted_at IS NULL
    LIMIT 1;

    -- Default due date +30 days for credit transactions without explicit date
    v_effective_due_date := COALESCE(
      p_due_date,
      CASE WHEN p_pay_method = 'credit'
        THEN p_transaction_date + INTERVAL '30 days'
      END
    );

    -- Insert AP/AR document
    ...
    -- Link via proper junction table (not the legacy linked_transaction_ids array)
