...
BEGIN
  ...
  -- Cesantía: use COMPLETE YEARS (matching Ministry of Labor interpretation)
  IF p_scenario = 'desahucio' THEN
    IF v_service_years >= 5 THEN
      v_cesantia_days := 23 * v_service_years;
  ...
  ELSE
    ...
    ELSE
      ...
      -- Round up to full entitlement when within 95% (matches Ministry behavior)
  ...
  -- Regalía proporcional (same as before)
  WITH raw_history AS (
    SELECT effective_date::date AS effective_date, salary::numeric AS salary
    FROM public.employee_salary_history
    WHERE employee_id = p_employee_id
