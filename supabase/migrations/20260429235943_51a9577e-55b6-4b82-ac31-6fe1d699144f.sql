...
BEGIN
  ...
  -- Vacation entitlement (Art. 177) — full-cycle ceiling
  ...
  ELSE
    ...
    ELSE
      -- Completed months in the current vacation cycle
      v_cycle_months := FLOOR(((p_termination_date - v_vacation_anchor + 1)::numeric / 30.4375))::int;

      IF v_total_service_months >= 60 THEN
        -- ≥ 5 years: 18-day cycle, prorated by completed months
        v_pending_vacation_days := CASE
          WHEN v_cycle_months >= 12 THEN 18
          WHEN v_cycle_months >=  5 THEN FLOOR(18.0 * v_cycle_months / 12.0)
          ELSE 0
        END;
      ELSE
        -- < 5 years: statutory monthly table (Art. 177)
        v_pending_vacation_days := CASE
  ...
  RETURN jsonb_build_object(
    ...
    'vacation_context', jsonb_build_object(
      ...
      'vacation_cycle_months_completed', v_cycle_months,
