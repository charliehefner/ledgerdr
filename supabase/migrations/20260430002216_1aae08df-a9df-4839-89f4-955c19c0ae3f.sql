...
BEGIN
  ...
  -- Vacation days: Art. 177 + 184 for ≥ 1 year service; Art. 179 + 180 for < 1 year
  ...
  ELSIF v_total_service_months >= 12 THEN
    -- Full statutory entitlement at termination (matches Ministerio de Trabajo calculator)
    v_pending_vacation_days := CASE
      WHEN v_total_service_months >= 60 THEN 18
      ELSE 14
    END;
  ELSE
    -- Less than 1 year of service: Art. 179 + 180 monthly scale
  ...
  year_segments AS (
    SELECT
      GREATEST(effective_date, DATE_TRUNC('year', p_termination_date)::date, v_employee.date_of_hire) AS segment_start,
      LEAST(
        COALESCE((LEAD(effective_date) OVER (ORDER BY effective_date) - INTERVAL '1 day')::date, p_termination_date),
        p_termination_date
      ) AS segment_end,
  ...
  RETURN jsonb_build_object(
    ...
    'total_service_months', v_total_service_months,
    'avg_monthly_salary', ROUND(v_avg_monthly_salary, 2),
    'avg_monthly_salary_12m', ROUND(v_avg_monthly_salary_12m, 2),
