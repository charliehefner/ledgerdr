-- Recreate calculate_payroll_for_period WITHOUT the /2 halving of recurring benefits.
-- Recurring values in employee_benefits.amount are PER PAY PERIOD, not per month.
...
BEGIN
  ...
  LOOP
    ...
    -- 3c. Benefits: period overrides first, then recurring AS-IS.
    -- IMPORTANT: employee_benefits.amount is PER PAY PERIOD (bi-monthly),
    -- NOT per month. Do NOT divide by 2. (Bug fixed permanently 2026-04-30.)
    ...
    -- 3d. Gross (absence is a deduction, NOT subtracted from gross)
    ...
    -- 3f. ISR (absence still reduces taxable income)
...
COMMENT ON FUNCTION public.calculate_payroll_for_period(uuid, boolean, uuid) IS
  'Payroll calculation. employee_benefits.amount is PER PAY PERIOD (bi-monthly). Do NOT divide by 2.';