...
BEGIN
  ...
  LOOP
    ...
    -- Overtime pays 1.35x hourly rate (base hour + 35% premium).
    -- base_pay (salary/2) only covers contractual hours: Mon-Fri 7:30-16:30 + Sat 7:30-11:30.
...
COMMENT ON FUNCTION public.calculate_payroll_for_period(uuid, boolean, uuid) IS
  'Payroll calculation. Overtime pays 1.35x hourly_rate (base + 35% premium) since base_pay only covers contractual hours (Mon-Fri 7:30-16:30 + Sat 7:30-11:30). Sunday hours pay 0.35x premium. employee_benefits.amount is PER PAY PERIOD (bi-monthly). Excludes employees not yet hired or already terminated relative to period dates.';