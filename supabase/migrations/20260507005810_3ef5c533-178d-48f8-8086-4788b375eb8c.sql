-- Drop prior 39-arg overload to avoid PostgREST ambiguity
DROP FUNCTION IF EXISTS public.create_transaction_with_ap_ar(
  date, text, text, text, numeric, text, text, date, numeric, numeric, numeric, text,
  text, text, text, text, numeric, boolean, text, text, text, text, text, date, numeric,
  text, uuid, text, numeric, numeric, text, numeric, numeric, numeric, numeric, numeric,
...
BEGIN
  ...
  IF v_should_create_ap_ar THEN
    ...
    INSERT INTO public.ap_ar_documents (
      ...
      v_ap_ar_account_id, p_entity_id, p_supplier_id, p_contract_id
    )
