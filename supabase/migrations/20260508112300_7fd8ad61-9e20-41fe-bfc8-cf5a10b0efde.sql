...
CREATE OR REPLACE FUNCTION public.create_transaction_with_ap_ar(
  ...
  p_name text DEFAULT NULL, p_rnc text DEFAULT NULL, p_comments text DEFAULT NULL,
  p_exchange_rate numeric DEFAULT NULL, p_is_internal boolean DEFAULT false,
  p_cost_center text DEFAULT 'general', p_transaction_direction text DEFAULT 'purchase',
  p_destination_acct_code text DEFAULT NULL, p_dgii_tipo_ingreso text DEFAULT NULL,
  p_dgii_tipo_bienes_servicios text DEFAULT NULL, p_due_date date DEFAULT NULL,
  p_destination_amount numeric DEFAULT NULL, p_itbis_override_reason text DEFAULT NULL,
  p_entity_id uuid DEFAULT NULL, p_ncf_modificado text DEFAULT NULL,
  p_monto_bienes numeric DEFAULT NULL, p_monto_servicios numeric DEFAULT NULL,
  p_dgii_tipo_retencion_isr text DEFAULT NULL, p_isc numeric DEFAULT 0,
  p_propina_legal numeric DEFAULT 0, p_otros_impuestos numeric DEFAULT 0,
  p_itbis_proporcionalidad numeric DEFAULT 0, p_itbis_al_costo numeric DEFAULT 0,
  p_itbis_percibido numeric DEFAULT 0, p_isr_percibido numeric DEFAULT 0,
...
BEGIN
  ...
  WHERE account_code = p_master_acct_code AND deleted_at IS NULL LIMIT 1;
  ...
  IF v_should_create_ap_ar THEN
    v_direction := CASE
      ...
      ELSE 'payable'
    END;
    v_doc_type := CASE WHEN v_is_advance THEN 'advance' ELSE 'invoice' END;
    v_acct_code := CASE
      WHEN v_is_advance THEN '1690'
      WHEN v_direction = 'receivable' THEN '1210'
      ELSE '2101'
    END;
    SELECT id INTO v_ap_ar_account_id FROM public.chart_of_accounts
    WHERE account_code = v_acct_code AND allow_posting = true AND deleted_at IS NULL LIMIT 1;
    v_effective_due_date := COALESCE(
      p_due_date,
      CASE WHEN p_pay_method = 'credit' THEN p_transaction_date + INTERVAL '30 days' END
    );

    -- Route through the unified create_ap_ar_document RPC so every doc shares
    -- the same validation, supplier auto-link, approval gate, and edit-lock.
    -- p_post_journal=false because the transaction itself produces its own journal.
    v_apar_result := public.create_ap_ar_document(
      p_direction       := v_direction,
      p_document_type   := v_doc_type,
      p_contact_name    := COALESCE(NULLIF(TRIM(p_name), ''), p_description),
      p_contact_rnc     := NULLIF(TRIM(COALESCE(p_rnc, '')), ''),
      p_document_number := v_legacy_id::TEXT,
      p_document_date   := p_transaction_date,
      p_due_date        := CASE WHEN v_is_advance THEN NULL ELSE v_effective_due_date END,
      p_currency        := p_currency,
      p_total_amount    := p_amount,
      p_notes           := p_description,
      p_account_id      := v_ap_ar_account_id,
      p_supplier_id     := p_supplier_id,
      p_contract_id     := p_contract_id,
      p_entity_id       := p_entity_id,
      p_offset_account_id := NULL,
      p_post_journal    := false,
      p_exchange_rate   := p_exchange_rate,
      p_user_id         := v_user_id
    );

    v_ap_ar_id := (v_apar_result ->> 'document_id')::uuid;
  ...
  RETURN jsonb_build_object(
    ...
    'ap_ar_document_id', v_ap_ar_id,
    'approval_status', COALESCE(v_apar_result ->> 'approval_status', 'not_required')
  );
END;
$$;
