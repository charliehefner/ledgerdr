...
BEGIN
  ...
  FROM public.bank_accounts WHERE id = p_bank_account_id AND is_active = true;
  ...
  IF v_service.ap_document_id IS NULL THEN
    ...
    SELECT * INTO v_service FROM public.service_entries WHERE id = p_service_entry_id;
  ELSE
    SELECT * INTO v_ap_doc FROM public.ap_ar_documents WHERE id = v_service.ap_document_id;
    IF NOT FOUND THEN RAISE EXCEPTION 'La cuenta por pagar enlazada no fue encontrada'; END IF;
  END IF;

  SELECT account_code INTO v_ap_account_code
  FROM public.chart_of_accounts WHERE id = v_ap_doc.account_id;

  -- Use the service's chosen expense account as the transaction master_acct_code
  -- so the expense is recognized in the correct GL account (not the AP control 2101).
  v_expense_code := COALESCE(NULLIF(TRIM(v_service.master_acct_code), ''), v_ap_account_code, '2101');
