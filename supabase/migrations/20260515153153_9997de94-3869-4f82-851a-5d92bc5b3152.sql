-- 1. Trigger function to keep employee_loans in sync with payroll_loan_deductions
CREATE OR REPLACE FUNCTION public.sync_loan_remaining_payments()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT number_of_payments INTO v_total
  FROM employee_loans
  WHERE id = v_loan_id;
  ...
  SET remaining_payments = GREATEST(v_total - v_count, 0),
      is_active          = (v_total - v_count) > 0,
      updated_at         = now()
  WHERE id = v_loan_id;
...
DROP TRIGGER IF EXISTS trg_sync_loan_remaining_payments ON public.payroll_loan_deductions;
CREATE TRIGGER trg_sync_loan_remaining_payments
AFTER INSERT OR DELETE ON public.payroll_loan_deductions
FOR EACH ROW EXECUTE FUNCTION public.sync_loan_remaining_payments();

-- 2. Backfill remaining_payments / is_active from real deduction history
UPDATE public.employee_loans el
SET remaining_payments = GREATEST(
      el.number_of_payments
        - COALESCE((SELECT COUNT(*) FROM public.payroll_loan_deductions pld WHERE pld.loan_id = el.id), 0),
      0
    ),
    is_active = (
      el.number_of_payments
...
-- 3. Rewrite calculate_payroll_for_period: date-driven loan eligibility, no more counter-decrement
...
BEGIN
  ...
  LOOP
    ...
    -- Date-driven loan deduction: include any active loan whose loan_date <= period end
    -- and which still has remaining installments based on real deduction history
    -- (excluding any deduction already recorded for THIS period to avoid double counting on re-commit).
    SELECT COALESCE(SUM(el.payment_amount), 0) INTO v_loan_ded
    FROM employee_loans el
    WHERE el.employee_id = v_emp.id
      AND el.loan_date <= v_period.end_date
      AND (
        SELECT COUNT(*) FROM payroll_loan_deductions pld
        WHERE pld.loan_id = el.id AND pld.period_id <> p_period_id
      ) < el.number_of_payments;
    ...
    IF p_commit THEN
      ...
      -- Insert the deduction row for this period (idempotent). Trigger will sync employee_loans.
      ...
      -- NOTE: no manual UPDATE on employee_loans here; trg_sync_loan_remaining_payments handles it.
    END IF;
